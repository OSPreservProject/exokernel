...
int init2_stats (char *server, int port, int group) {
     ...
     localSock.sin_port = htons (9328);

     assert (bind (remoteFd, &localSock, sizeof (localSock)) >= 0);
...
int init2_graphs (int numGraphs, char *titles, char *legends, int maxRange, 
...
int stats2_barrier (char *barrierstr) {
     int ret;
     int dummy;
