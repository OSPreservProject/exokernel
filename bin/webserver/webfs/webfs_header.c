...
int web_header_datezeroed (char *buf, int buflen)
{
   char datebuf[64];
   int len;
...
int web_header_contenttype (char *buf, int buflen, int type)
{
   ...
   if (type == -1) {
      ...
      mimetype = NULL;
      for (len=0; len<MimeTypeCount; len++) {
         if (type == MimeTypes[len].typeno) {
            mimetype = MimeTypes[len].type;
            break;
         }
      }
...
int web_header_contentencoding (char *buf, int buflen, int type)
{
   ...
   mimeencoding = NULL;
   for (len=0; len<MimeEncodingCount; len++) {
      if (type == MimeEncodings[len].typeno) {
         mimeencoding = MimeEncodings[len].type;
         break;
      }
   }
   assert (mimeencoding != NULL);
/*
...
int web_header_contentlength (char *buf, int buflen, unsigned int filelen, int adjust)
{
   char lenbuf[32];
   int len;
