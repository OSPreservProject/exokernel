...
int len;
{
	...
	if (S_ISSOCK(statbuf.st_mode))
	    ...
	    /* ELW -- cast tokenbuf to struct sockaddr * */
...
#else
#ifdef HAS_FCNTL
    /* ELW -- cast func to int */
	retval = fcntl(fileno(stio->ifp), (int)func, (int)s);
#else
	fatal("fcntl is not implemented");
#endif
#endif
...
#endif
    /* ELW -- cast addr to struct sockaddr * */
...
int *arglast;
{
    ...
    switch (optype) {
    case O_GETSOCKNAME:
      /* ELW -- cast st[sp]->str_ptr to struct sockaddr * */
