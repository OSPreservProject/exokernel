...
void do_sound(Bit16u period) {
  if ((port61 & 3) == 3 && (pit[2].mode == 2 || pit[2].mode == 3)) {
     if (config.console && !config.X) {
        debug_io("SPEAKER: on, period=%d\n",pit[2].write_latch);
        /* do_ioctl(console_fd,KIOCSOUND,pit[2].write_latch & 0xffff); */  /* XXX sound */
     }
     else {
        debug_io("SPEAKER: Dumb emulated beep\n");
        putchar('\007');
     }
  }
  else if (config.console && !config.X) {
     debug_io("SPEAKER: sound OFF!\n");
     /* do_ioctl(console_fd,KIOCSOUND,0); */  /* XXX sound */
  }
}
...
#if 0 /* for debugging */
  debug_io("pit_latch:  latched value 0x%4.4x for timer %d\n",
...
Bit8u pit_inp(Bit32u port)
{
...
#if 1
  if (port >= PIT_TIMERS) {
      printf("port = 0x%x   PIT_TIMERS=%d\n", port, PIT_TIMERS);
  }
#endif
  ASSERT(port < PIT_TIMERS);
  ...
  if (port == 1)
    debug_io("PORT:  someone is reading the CMOS refresh time?!?");
...
#if 0
  debug_io("PORT: pit_inp(0x%x) = 0x%x\n", port+0x40, ret);
#endif
  return ret;
}
...
void pit_outp(Bit32u port, Bit8u val)
{
    ...
    port -= 0x40;
    if (port == 1) {
	debug_io("PORT: someone is writing the CMOS refresh time?!?");
    ...
    trace_io("PORT: pit_outp(0x%x, 0x%x)\n", port+0x40, val);
...
#ifndef MONOTON_MICRO_TIMING
		int tick_rate = CLOCK_TICK_RATE;
#else /* MONOTON_MICRO_TIMING */
		int tick_rate = PIT_TICK_RATE;
#endif /* MONOTON_MICRO_TIMING */
		debug_io("timer_interrupt_rate requested %.3g Hz, granted %.3g Hz\n",
			 tick_rate/(double)pit[0].cntr, 10000.0/timer_div);
	    }
	}
...
#if 0
      debug_io("PORT: writing outp(0x43, 0x%x)\n", val);
#endif
      break;
    case 3:
...
#ifdef NEW_KBD_CODE
  port61 = 0;
  /* do_ioctl(console_fd,KIOCSOUND,0); */    /* sound off */  /* XXX sound */
#endif
}
#endif /* new engine */
