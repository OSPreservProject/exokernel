...
Bit16u port_inw(Bit32u port)
{
    ...
    if (emu_handler(port).read_portw != 0) {
...
void port_outw(Bit32u port, Bit16u word)
{
    ...
    if (emu_handler(port).write_portw != 0) {
...
void port_init(void)
{
    ...
    port_handler[0].read_portw = 0;
    port_handler[0].write_portw = 0;
    ...
    port_handler[1].read_portw = 0;
    port_handler[1].write_portw = 0;
    ...
    port_handler[2].read_portw = 0;
    port_handler[2].write_portw = 0;
    ...
    port_handler[3].read_portw = 0;
    port_handler[3].write_portw = 0;
	...
	port_handler[i].read_portb = 0;
	port_handler[i].write_portb = 0;
	port_handler[i].read_portw = 0;
	port_handler[i].write_portw = 0;
    }
...
void port_register_handler(emu_iodev_t device)
{
    ...
    i_printf("PORT: registering \"%s\" for port range [0x%04x-0x%04x]\n",
	     device.handler_name, device.start_addr, device.end_addr);

    if (device.irq != EMU_NO_IRQ && device.irq >= EMU_MAX_IRQS) {
	dprintf("PORT: IO device %s registered with IRQ=%d above %u\n",
	...
	if (port_handles >= EMU_MAX_IO_DEVICES) {
	    dprintf("PORT: too many IO devices, increase EMU_MAX_IO_DEVICES\n");
	    leaveemu(ERR_IO);
	}
	if (device.irq != EMU_NO_IRQ && irq_handler_name[device.irq]) {
	    dprintf("PORT: IRQ %d conflict.  IO devices %s & %s\n",
	...
	if (port_handler_id[i] != 0) {
	    dprintf("PORT: IO device address conflict at IO address 0x%x\n", i);
	    dprintf("      conflicting devices: %s & %s\n",
...
void print_port(u_int port)
{
    printf("port 0x%x is handled by...\n", port);
    printf("port id:      0x%x\n", port_handler_id[port]);
    printf("handler name: %s\n", emu_handler(port).handler_name);
    printf("read_portb:   0x%08x\n", (u_int)emu_handler(port).read_portb);
    printf("write_portb:  0x%08x\n", (u_int)emu_handler(port).write_portb);
}
