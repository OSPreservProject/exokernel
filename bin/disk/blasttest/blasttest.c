...
int main (int argc, char *argv[])
{
   ...
   buf = malloc (MAX_NUMSECTS * SECT_SIZE);
   assert (buf != NULL);
   bzero (buf, (MAX_NUMSECTS * SECT_SIZE));
...
printf ("Read Bandwidth\n");
   ...
   for (sects = 1; sects <= TEST_MAXSECTS; sects *= 2) {
      ...
      for (i = START_SECT; (i+sects) <= (START_SECT + MAX_NUMSECTS); i += sects) {
         ...
         reqcount++;
         if (sys_disk_request (&__sysinfo.si_pxn[diskno], &theBuf, CAP_ROOT)) {
            printf ("Unable to initiate request: sects %d, i %d\n", sects, i);
            exit (1);
         }

         /* watch for overkill */
         while ((*((volatile int *)&theBuf.b_resid) + reqcount) >= 100) ;

      }

      /* poll for completion */
      while (*((volatile int *)&theBuf.b_resid) != -reqcount) ;
