...
/* Insert a direct block */
void ffs_db_insert(struct xn_op *op, int index, db_t db, cap_t cap) {
	ffs_write_db(op, index, db, cap, 0);
}
/* Free a direct block. */
...
int xn_import(struct udf_type *t) {
	...
	if(sys__xn_mount(&r, t->type_name, c) == XN_SUCCESS) {
		...
		for(ppn = -1, i = 0; i < r.nelem; i++, ppn++) {
			ppn = sys__xn_bind(r.db+i, ppn, 0, XN_FORCE_READIN, 1);
			if(ppn < 0) {
				printf("ppn = %ld\n", ppn);
				fatal(Death);
			}
...
int ffs_dalloc(u_inode *ui, int n, db_t hint) { 
	...
	res = sys__xn_alloc(ui->da, &op, XN_NIL);
...
int ffs_dinsert(u_inode *ui, int n, db_t hint) { 
	xn_err_t res;
	xn_op op;

        ffs_db_insert(&op, n, hint, ui->uid);
	res = sys__xn_insert_attr(ui->da, &op);

	/* Sanity check: read from xn. */
        if(res == XN_SUCCESS)
	  demand(ui->incore.db[n] == op.u.db, Insert sucks);
	return res;
}
...
int ffs_dfree(u_inode *ui, int n) { 
	...
	db = ui_get_db(ui, n);
	return ffs_realloc(&op, sys__xn_free, ui->da, ui->uid, n, db, XN_DB); 
}
...
int ffs_free_block(u_inode *ui, size_t i) {
        ...
        return sys__xn_free(ui->da, &op, XN_NIL);
}
...
int ffs_alloc_store(u_inode *ui, size_t i, ppn_t hint) {
	...
	struct xn_op op;
        ...
        ppn = sys__xn_bind(ui_get_db(ui, i), -1, 0, XN_ALLOC_ZERO, 1);
        if(ppn > 0)
                ui->store[i] = ppn;
	else if (ppn == XN_REGISTRY_MISS) {
	  ffs_db_insert(&op, i, ui_get_db(ui, i), ui->uid);
	  ppn = sys__xn_insert_attr(ui->da, &op);
	  if (ppn == XN_SUCCESS)
	    return ffs_alloc_store(ui, i, hint);
	}
        return ppn;
}

int ffs_write_block(u_inode *ui, size_t index, size_t n, xn_cnt_t *cnt) {
	return sys__xn_writeback(ui_get_db(ui, index), n, cnt);
}
int ffs_read_block(u_inode *ui, size_t index, size_t n, xn_cnt_t *cnt) {
	return sys__xn_readin(ui_get_db(ui, index), n, cnt);
}

int ffs_write_bytes(u_inode *ui, size_t index, void *addr, size_t nbytes) {
        return sys__xn_writeb(ui_get_da(ui, index), addr, nbytes, ui->uid);
}

int ffs_read_bytes(u_inode *ui, size_t index, void *addr, size_t nbytes) {
        return sys__xn_readb(addr, ui_get_da(ui, index), nbytes, ui->uid);
}

int ffs_free_store(u_inode *ui, size_t i) {
        return sys__xn_unbind(ui_get_db(ui, i), ui_get_store(ui, i), ui->uid);
}

int ffs_write_data(u_inode *ui, size_t offset, void *addr, size_t nbytes) {
        return sys__xn_writeb(ui->da+offset-byte_off, addr, nbytes, ui->uid);
}

int ffs_read_data(u_inode *ui, size_t offset, void *addr, size_t nbytes) {
        return sys__xn_readb(addr, ui->da+offset-byte_off, nbytes, ui->uid);
}
#define offset(field) offsetof(struct inode, field)

int ffs_set_type(u_inode *ui, int ty, cap_t cap) {
     return sys__xn_set_type(ui->da + offset(type) - byte_off, ty, &ty, sizeof ty, cap);
}
