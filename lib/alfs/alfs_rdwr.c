...
int alfs_readBuffer (inode_t *inode, char *buffer, unsigned offset, unsigned length)
{
     ...
     if (offset >= alfs_inode_getLength(inode)) {
        printf ("alfs_readBuffer: trying to read past end of file (offset %d, length %d)\n", offset, alfs_inode_getLength(inode));
        return (ALFS_ELENGTH);
     }
...
int alfs_writeBuffer (inode_t *inode, char *buffer, unsigned offset, unsigned length)
{
     block_num_t currentBlock;
     buffer_t *blockBuffer;
     int amountToWrite;
...
/*
printf ("alfs_writeBuffer: inodeNum %d, filesize %d, offset %d, length %d\n", inode->inodeNum, alfs_inode_getLength(inode), offset, length);
*/
...
*/
     ...
     while (length) {
	  ...
	  if ((amountToWrite < min(BLOCK_SIZE, alfs_inode_getLength(inode))) || (blockOffset != 0)) {
