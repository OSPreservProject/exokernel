   ...
   if (twtcb) {
	...
	/* compute netcardno from received packet's dst_ether addr */
      ...
      if (xio_tcp_closed (&tmptcb)) {
         if (twtcb->demux_id) {
            int ret = xio_net_wrap_freedpf (nwinfo, twtcb->demux_id);
            assert (ret == 0);
         }
...
static inline void xio_tcpcommon_TWtimeout (xio_twinfo_t *twinfo, xio_nwinfo_t *nwinfo)
{
   timewaiter_t *done;
   ...
   while ((done = twinfo->timewait_queue.tqh_first) && 
     ...
     if (done->demux_id) {
       int ret = xio_net_wrap_freedpf (nwinfo, done->demux_id);
       assert (ret == 0);
     }
