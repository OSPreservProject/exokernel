...
static void xio_tcp_preparePacket (struct tcb *tcb, int flag, uint32 seqno, int len, int len2, int datasum)
{
   ...
   if (tcb->send_ready & (~TCP_FL_ACK)) {
      kprintf ("send_ready %x, flag %x, len %d, len2 %d\n", tcb->send_ready, flag, len, len2);
   }
   ...
   tcb->outpackets++;
/*
   kprintf ("(%d) send: len %d, flags %x\n", sys_geteid(), (len+len2), flag);
   kprintf ("send_segment %p: datalen %d, len2 %d, seqno %u ackno %u wnd %u, flags %x\n", tcb, len, len2, tcb->snd_next, tcb->rcv_next, tcb->rcv_wnd, flag);
*/
...
void xio_tcp_prepCtlPacket (struct tcb *tcb)
{
   if ((tcb->state != TCB_ST_LISTEN) && (tcb->state != TCB_ST_TIME_WAIT) && (tcb->state < TCB_ST_CLOSED) && (tcb->state != TCB_ST_SYN_SENT) && (tcb->state != TCB_ST_SYN_RECEIVED)) {
      ...
      kprintf ("xio_tcp warning: prepCtlPacket called for wierd state (%d)\n", tcb->state);
   }
}
...
/*
   kprintf ("send_resp: seqno %d, ackno %d, flag %x\n", seqno, ackno, flag);
*/
...
/*
kprintf ("xio_tcp_senddata: tcb %p, addr %p, sz %d, addr2 %p, sz2 %d, flags %x, checksums %p\n", tcb, addr, sz, addr2, sz2, flags, checksums);
*/
   if (n == 0) {
      if (close) {
   ...
   if ((!xio_tcp_writeable(tcb)) && (!xio_tcp_rewriting(tcb))) {
      kprintf ("preparing a data packet from a non-standard TCP state (%d)\n", tcb->state);
      kprintf ("snd_next %d, snd_max %d, sz %d, sz2 %d, send_offset %d\n", tcb->snd_next, tcb->snd_max, sz, sz2, tcb->send_offset);
      kprintf ("n %d, snd_una %d, snd_wnd %d, flags %x, close %d, mss %d\n", n, tcb->snd_una, tcb->snd_wnd, flags, close, tcb->mss);
   }
#endif
	...
	/* GROK -- will this function correctly if one of the lens is odd */
      ...
      if (flag & TCP_FL_FIN) {
         ...
         if (tcb->state == TCB_ST_CLOSE_WAIT) {
            DPRINTF(2, ("xio_tcp_senddata %p: go to LAST_ACK\n", tcb));
            tcb->state = TCB_ST_LAST_ACK;
         } else if (tcb->state == TCB_ST_ESTABLISHED) {
            DPRINTF(2, ("xio_tcp_senddata %p: go to FIN_WAIT1\n", tcb));
            tcb->state = TCB_ST_FIN_WAIT1;
         } else if (xio_tcp_finsent(tcb)) {
            DPRINTF(2, ("xio_tcp_senddata %p: resending FIN (state %d)\n", tcb, tcb->state));
	    //            kprintf ("xio_tcp_senddata %p: resending FIN (state %d)\n", tcb, tcb->state);
         } else {
            DPRINTF(2, ("xio_tcp_senddata %p: stay in current state (%d)\n", tcb, tcb->state));
            kprintf ("xio_tcp_senddata %p: must be retransmitting if in state %d\n", tcb, tcb->state);
         }
      }
      ...
      //kprintf ("sending a window probe...\n");
...
void xio_tcp_initiate_close(struct tcb *tcb)
{
   ...
   if (!xio_tcp_writeable(tcb)) {
     //      kprintf("xio_tcp_initiate_close: illegal state %d (tcb %p)\n", tcb->state, tcb);
   ...
   if (tcb->state == TCB_ST_CLOSE_WAIT) {
      DPRINTF(2, ("xio_tcp_initiate_close %p: go to LAST_ACK\n", tcb));
      tcb->state = TCB_ST_LAST_ACK;
   } else if (tcb->state == TCB_ST_ESTABLISHED) {
      DPRINTF(2, ("xio_tcp_initiate_close %p: go to FIN_WAIT1\n", tcb));
      tcb->state = TCB_ST_FIN_WAIT1;
   }
...
void xio_tcp_initiate_connect (struct tcb *tcb, int srcportno, int dstportno, char *ip_addr, char *eth_addr)
{
   struct ip *ip = ((struct ip *) &tcb->snd_recv_r0_data[XIO_EIT_IP]);
/*
kprintf ("xio_tcp_initiate_connect: srcportno %d, dstportno %d\n", srcportno, dstportno);
*/
...
static int connect_timeout(struct tcb *tcb)
{
   ...
   //kprintf("retransmit connect (state %d)\n", tcb->state);
...
{
   //kprintf ("(%d) write_timeout (retries %d, snd_iss %d, snd_next %d, snd_max %d, snd_una %d)\n", sys_geteid(), tcb->retries, tcb->snd_iss, tcb->snd_next, tcb->snd_max, tcb->snd_una);
...
{
   //kprintf ("close_timeout %p (state %d, retries %d)\n", tcb, tcb->state, tcb->retries);
...
{
   int ret = 0;
   DPRINTF(2, ("tcb %p: time out; state %d\n", tcb, tcb->state));

   //kprintf("timeout %p %d (src %d, dst %d) (in %d, out %d)\n", tcb, tcb->state, ntohs(tcb->tcpsrc), ntohs(tcb->tcpdst), tcb->inpackets, tcb->outpackets, tcb->outbuffers);
...
static int process_syn_ack(struct tcb *tcb, char *packet)
{
   ...
   if (tcp->flags & TCP_FL_SYN) {
      ...
      kprintf ("process_syn_ack: unexpected SYN in state %d\n", tcb->state);
...
static int process_data(struct tcb *tcb, char *packet, int fin, int flags)
{
   ...
   if (SEQ_LEQ(tcp->seqno, tcb->rcv_next) && SEQ_GEQ(tcp->seqno + datalen, tcb->rcv_next)) {
	...
	/* Segment contains data past rcv_next, keep it.  For now, drop it. */
      ...
      DPRINTF(2, ("process_data %p: segment seq %d is out of order n %d\n", tcb, tcp->seqno, tcb->rcv_next));
   } else {
      kprintf ("tcb %p, state %d, tcpflags %x, seqno %d, rcv_next %d datalen %d\n", tcb, tcb->state, tcp->flags, tcp->seqno, tcb->rcv_next, datalen);
...
static void st_syn_sent(struct tcb *tcb, char *packet, int flags)
{
   ...
   if ((tcp->flags & TCP_FL_ACK) && (SEQ_LEQ(tcp->ackno, tcb->snd_iss) ||
      ...
      STINC(tcpstats, nrstsnd);
      kprintf ("st_syn_sent: bad ACK on SYN\n");
      xio_tcp_preparePacket(tcb, TCP_FL_RST|TCP_FL_ACK, tcp->ackno, 0, 0, 0);
      tcb->send_important = 0;
      return;
   }
   ...
   if (tcp->flags & TCP_FL_RST) {
      STINC(tcpstats, nrstrcv);
      tcb->state = TCB_ST_CLOSED;
kprintf ("st_synsent: RST received -- dropping connection\n");
      return;
   }
...
*/
   ...
   if ((tcp->flags & TCP_FL_ACK) && SEQ_GEQ(tcp->ackno, tcb->snd_una) && SEQ_LEQ(tcp->ackno, tcb->snd_next)) {
      ...
      kprintf ("st_syn_received: send RST\n");
...
static void st_time_wait(struct tcb *tcb, char *packet, int flags)
{
   ...
   if ((len != 0) || (tcp->seqno != tcb->rcv_next)) {
      ...
      if (tcp->flags & TCP_FL_SYN) {
	//         kprintf ("dropping SYN packet for connection in TIME_WAIT state\n");
      }
      return;
   }

   if (tcp->flags & TCP_FL_RST) {
     //      kprintf ("(%d) got a RST, so get rid of the timewaiter! (packet %p)\n", __envid, packet);
      tcb->state = TCB_ST_CLOSED;
      return;
   }
...
int xio_tcp_packetcheck (char *packet, int len, int *datasum)
{
   ...
   if (len < XIO_EIT_DATA) {
      kprintf ("packet too short (len %d)\n", len);
      return (0);
   }
   ...
   if (ipsum != 0) {
      ...
      if (sum2 != ipsum) {
         ...
         kprintf ("xio_tcp_packetcheck: bad ip checksum (sum %x, rcv_ip_checksum %x\n", sum2, ipsum);
         return(0);
      }
   }
   ...
   if (totallength > len) {
	// || rcv_ip->fragoffset != 0) {
      ...
      kprintf ("tcp_proces_packetcheck: segment with len %d (packetlen %d) off %d is not complete\n", totallength, len, ntohs(rcv_ip->fragoffset));
      return(0);
   }
   ...
   if (rcv_tcp->cksum != 0) {
      ...
      if (sum2 != tcpsum) {
         ...
         kprintf ("xio_tcp_packetcheck: bad tcp inet checksum: sum %x, rcv_tcp->cksum %x, tcplen %d, totallen %d, data %p, packet %p\n", sum2, tcpsum, n, totallength, data, packet);
         kprintf ("ackno %d, seqno %d, tcpdst %x, tcpsrc %x\n", (uint)ntohl(rcv_tcp->ackno), (uint)ntohl(rcv_tcp->seqno), ntohs(rcv_tcp->dst_port), ntohs(rcv_tcp->src_port));
         return(0);
      }
   } else if (datasum) {
      *datasum = -1;
...
void xio_tcp_handlePacket(struct tcb *tcb, char *packet, uint flags)
{
   ...
   DPRINTF (2, ("tcp_process: tcb %p, r %p, tcp %p, ip %p\n", tcb, packet, tcp, rcv_ip));
...
/*
kprintf ("tcp_process: tcb %p, r %p, tcp %p, ip %p\n", tcb, packet, tcp, rcv_ip);
kprintf ("more info: src_port %d, ackno %u, seqno %u, window %u, flag %x\n", (uint)ntohs(tcp->src_port), tcp->ackno, tcp->seqno, tcp->window, tcp->flags);
kprintf ("more info: snd_una %d, snd_wnd %d, snd_wls %d, snd_wla %d\n", tcb->snd_una, tcb->snd_wnd, tcb->snd_wls, tcb->snd_wla);
kprintf ("more info: rcv_next %d, snd_next %d\n", tcb->rcv_next, tcb->snd_next);
kprintf ("recv: ackno %d, snd_una %d, seqno %d, flag %x\n", tcp->ackno, tcb->snd_una, tcp->seqno, tcp->flags);
*/
DPRINTF (2, ("tcp_process: src_port %d, ackno %u, seqno %u, window %u, flag %x\n", (uint)ntohs(tcp->src_port), tcp->ackno, tcp->seqno, tcp->window, tcp->flags));
DPRINTF (4, ("for comparison: snd_una %d, snd_wnd %d, snd_wls %d, snd_wla %d\n", tcb->snd_una, tcb->snd_wnd, tcb->snd_wls, tcb->snd_wla));
...
int xio_tcp_handleListenPacket(struct tcb *new_tcb, char *packet)
{
   ...
   struct ip *new_ip;
/*
kprintf ("xio_tcp_handleListenPacket: src_port %d, ackno %u, seqno %u, window %u\n", (uint)ntohs(rcv_tcp->src_port), ntohl(rcv_tcp->ackno), ntohl(rcv_tcp->seqno), ntohl(rcv_tcp->window));
*/

   /* Check for rst and ignore */
   if ((rcv_tcp->flags & (TCP_FL_RST|TCP_FL_ACK)) || (!(rcv_tcp->flags & TCP_FL_SYN))) {
