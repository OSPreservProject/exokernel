...
char *xio_tcpsocket_default_pagealloc (void *refptr, int size)
{
...
void xio_tcpsocket_initinfo (xio_tcpsock_info_t *info, char * (*pagealloc)(void *, int), int usetwinfo)
{
   ...
   if (usetwinfo) {
	...
	/* real setup must be external... */
   xio_net_wrap_init (&info->nwinfo, NULL, 0);
}
...
void xio_tcpsocket_freesock (struct tcpsocket *tcpsock)
{
   ...
   if (tcpsock->demux_id != -1) {
      int ret = xio_net_wrap_freedpf (&info->nwinfo, tcpsock->demux_id);
      assert (ret == 0);
      tcpsock->demux_id = -1;
   }
...
struct tcpsocket * xio_tcpsocket_allocsock (xio_tcpsock_info_t *info)
{
   ...
   if (tcpsock) {
      ...
      assert ((tcpsock->livenext == NULL) && (tcpsock->liveprev == NULL));
...
void xio_tcpsocket_handlePackets (xio_tcpsock_info_t *info)
{
   ...
   while ((packet = xio_net_wrap_getPacket(&info->nwinfo)) != NULL) {
	 ...
	 /* try to combine this with data/close sends below! */
      ...
      } else if (xio_tcpcommon_handleTWPacket (&info->twinfo, packet->r[0].data, &info->nwinfo)) {
      } else {
         int netcardno;
         struct tcb *listentcb = xio_tcpcommon_findlisten (&info->dmxinfo, packet->r[0].data, &info->nwinfo, &netcardno);
         if (listentcb) {
            tcb =  (listentcb) ? xio_tcpsocket_gettcb(listentcb) : NULL;
            if (tcb != NULL) {
               ret = xio_tcp_handleListenPacket (tcb, packet->r[0].data);
               assert (ret != 0);
               tcb->netcardno = xio_tcpsocket_getnetcardno (&info->nwinfo, packet->r[0].data, packet->r[0].sz);
	       ...
	       assert (ret == 0);
            ...
            xio_tcpcommon_handleRandPacket (packet->r[0].data, &info->nwinfo);
         }
      }
      xio_net_wrap_returnPacket (&info->nwinfo, packet);
   }
   ...
   if (curtime > info->lasttime) {
	 ...
	 /* try to combine this with data/close sends below! */
      ...
      xio_tcpcommon_TWtimeout (&info->twinfo, &info->nwinfo);
...
static int xio_tcpsocket_tcpclose (struct tcpsocket *sock, int noblock)
{
   ...
   while (sock->write_offset > sock->tcb.send_offset) {
      ...
      xio_tcp_waitforChange (&sock->info->nwinfo, 
...
int xio_tcpsocket_close (struct tcpsocket *sock, int noblock)
{
   ...
   FDPRINT (("xio_tcpsocket_close: demux_id %d, noblock %d\n",sock->demux_id, noblock));

   if (sock->tcb.state == TCB_ST_LISTEN) {
/*
      kprintf ("closing a listen socket\n");
*/
...
int xio_tcpsocket_bind(struct tcpsocket *sock, struct sockaddr *reg_name, int namelen)
{
   ...
   /* HBXX - somewhere here we should check that the local address is valid, like
    0.0.0.0 (choose any), 127.0.0.1, 18.... etc... grep look at udp_socket.c */

#if 0	/* GROK - this is wrong. the copy goes in the other way, if at all */
   memcpy((char *)&name->sin_addr,ip_my_addr,4);
#endif

   name->sin_port = htons(_src_port);
...
int xio_tcpsocket_write(struct tcpsocket *sock, char *buffer, int length, int nonblocking) {
   int retlen = 0;
...
*/
      ...
      if (retlen < length) {
         xio_tcp_waitforChange (&sock->info->nwinfo, sock->tcb.timer_retrans);
         xio_tcpsocket_handlePackets (sock->info);
      }
   }
/*
...
int xio_tcpsocket_connect(struct tcpsocket *sock, struct sockaddr *uservaddr, int namelen, int flags, int nonblock, uint16 _src_port, char *src_ip, char *dst_ip, char *src_eth, char *dst_eth) {
   struct sockaddr_in *name = (struct sockaddr_in *) uservaddr;
   ...
   if (xio_tcpsocket_skipdpf == 0) {
      sock->demux_id = xio_net_wrap_getdpf_tcp (&sock->info->nwinfo, _src_port, ntohs(name->sin_port));
      if (sock->demux_id < 0) {
   ...
   sock->tcb.netcardno = xio_tcpsocket_getnetcardno (&sock->info->nwinfo, src_eth, 6);
...
//kprintf ("(%d) xio_tcpsocket_listen: backlog %d, portno %d\n", getpid(), backlog, ntohs(sock->tcb.tcpsrc));
   ...
   sock->demux_id = xio_net_wrap_getdpf_tcp (&sock->info->nwinfo, portno, -1);
...
void xio_tcpsocket_exithandler (xio_tcpsock_info_t *info)
{
   while (info->livelist) {
      if (xio_tcp_closed (&info->livelist->tcb)) {
         xio_tcpsocket_freesock (info->livelist);
      } else {
         xio_tcp_waitforChange (&info->nwinfo, 
				info->livelist->tcb.timer_retrans);
         xio_tcpsocket_handlePackets (info);
      }
   }

   while (info->twinfo.timewaiter_cnt != 0) {
      sleep (5);
      xio_tcpsocket_handlePackets (info);
   }
   xio_net_wrap_shutdown (&info->nwinfo);
}
...
int xio_tcpsocket_select_pred(struct tcpsocket *sock, int rw, struct wk_term *t)
{
   ...
   if (next != -1) {
      next = xio_net_wrap_wkpred_packetarrival (&sock->info->nwinfo, t);
   }
...
int xio_tcpsocket_pred (xio_tcpsock_info_t *info, struct wk_term *t)
{
   int next;

   FDPRINT (("xio_tcpsocket_pred: info %p\n", info));

   next = xio_net_wrap_wkpred_packetarrival (&info->nwinfo, t);
