...
*/
   ...
   if (chkuniq) {
	 ...
	 //printf ("new connection equals existing connection: temp %x, ipsrc %x, ipdst %x, tcpsrc %d, tcpdst %d\n", (uint) temp, temp->ipsrc, temp->ipdst, temp->tcpsrc, temp->tcpdst);
	 return (-1);
       }
     }
   }
...
/*
   kprintf ("xio_tcp_demux_search: ipsrc %x, ipdst %x, tcpsrc %d, tcpdst %d, entry %d, temp %x\n", ipsrc, ipdst, rcv_tcp->src_port, rcv_tcp->dst_port, entry, (uint)temp);
*/
