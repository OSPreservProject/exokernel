...
#ifndef __VOS_UDP_FD_OPS_H__
#define __VOS_UDP_FD_OPS_H__
...
extern int 
fd_udp_read(S_T(fd_entry) *fd, void *buffer, int nbyte);

extern int 
fd_udp_recvfrom(S_T(fd_entry) *fd, void *buffer, size_t length, 
                unsigned flags, struct sockaddr *rfrom, int *rfromlen);

extern int 
fd_udp_write(S_T(fd_entry) *fd, const void *buffer, int nbyte);
