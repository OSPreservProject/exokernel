...
udp_open(struct udp_session *s, 
    ...
    if(((int)(s->demux_id = sys_self_dpf_insert_old(CAP_ROOT, 0, filter, sz, 0))) < 0)
	demand(0, error!);

    ae_eth_init();

    s->pkt = ae_eth_get_pkt();
    assert(s->pkt);
    s->pkt->flag = -1;
    ae_eth_poll(s->demux_id, s->pkt);
}    


/* close a udp session */
int 
udp_close(struct udp_session *s) {
 ...
 */
int 
udp_read(struct udp_session *s, void *addr, uint16 sz) {
    ...
    ip = (struct ip_pkt *) (s->pkt->recv.r[0].data + sizeof (struct ether_pkt));
    udp = (struct udp_pkt *) (s->pkt->recv.r[0].data + sizeof (struct ether_pkt) + sizeof (struct ip_pkt));
    ...
    sz = MIN(len, sz);
    memcpy(addr, (s->pkt->recv.r[0].data + sizeof (struct ether_pkt) + sizeof (struct ip_pkt) + sizeof (struct udp_pkt)), sz);

    len = ae_eth_poll(s->demux_id, s->pkt);
    assert(len >= 0);
 ...
 */
int 
udp_write(struct udp_session *s, void *addr, uint16 sz) {
    register struct ip_pkt *ip;
    register struct udp_pkt *udp;
    int len;
