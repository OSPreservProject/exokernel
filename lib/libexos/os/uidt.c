...
{
  if (_exos_self_insert_pte (0, PG_U | PG_W | PG_P, va, ESIP_DONTPAGE, NULL))
    return (0);
  return (1);
}
...
{
  kprintf("[trap %d received. eip 0x%08x]\n", trapno, eip);
}
...
void uidt_initialize(void) 
{
  ...
  for (i=uidt_start; i < uidt_start + uidt_size; i+= NBPG) {
    ...
    assert(mprotect((caddr_t)i,NBPG,PROT_READ | PROT_WRITE) != -1);
  }
