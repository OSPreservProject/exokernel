...
int
check_magic(u_int magic, int *layout)
{
  u_int mid,  /* machine id */
    mag,      /* magic number */
    midmag;   /* combined */

  /* check for interpreter */
  if (!strncmp("#!", (char*)&magic, 2)) {
    *layout = BF_UNKNOWN;
  ...
  if (*layout != BF_UNKNOWN) return BF_AOUT_386BSD;
  ...
  /* only recognize the 386 version of openbsd */
  if (mid == MID_I386) {
    switch (midmag) {
    ...
    if (*layout != BF_UNKNOWN) return BF_AOUT_OPENBSD;
  }
