...
#undef memset
#define memset(a,b,c) memset14(a,b,c)
/* assumption len < 4, and c = 0 */
...
inline int *xdr_encode_fhandle(int *p, void *fhandle)
{
...
inline int *xdr_decode_fhandle(int *p, void *fhandle)
{
...
inline int *xdr_encode_string(int *p, const char *string)
{
  int len, quadlen;
...
inline int *xdr_decode_string(int *p, char *string, int maxlen)
{
  unsigned int len;
...
inline int *xdr_encode_data(int *p, char *data, int len)
{
  int quadlen;
...
inline int *xdr_decode_data(int *p, char *data, int *lenp, int maxlen)
{
  unsigned int len;
...
inline int *xdr_decode_fattr(int *p, struct nfs_fattr *fattr)
{
...
inline int *xdr_encode_sattr(int *p, struct nfs_sattr *sattr)
{
  *p++ = htonl(sattr->mode);
  *p++ = htonl(sattr->uid);
  *p++ = htonl(sattr->gid);
...
inline int *xdr_decode_entry(int *p, struct nfs_entry *entry)
{
...
inline int *xdr_decode_fsinfo(int *p, struct nfs_fsinfo *res)
{
  res->tsize = ntohl(*p++);
  res->bsize = ntohl(*p++);
  res->blocks = ntohl(*p++);
