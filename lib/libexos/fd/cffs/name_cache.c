...
nc_t * name_cache_init (int size, int buckets)
{
   ...
   namecache = (nc_t *) __malloc (size);
   entries = name_cache_entries(size);
...
void name_cache_addEntry (nc_t *namecache, 
			  int id, char *name, int namelen, int value)
{
   nc_entry_t *entry;
...
int name_cache_findEntry (nc_t *namecache, 
			  int id, char *name, int namelen, int *valueP)
{
   nc_entry_t *entry;
   int checks = 0;
...
void name_cache_removefunc(nc_rm_t f) {
  rm_func = f;
}
