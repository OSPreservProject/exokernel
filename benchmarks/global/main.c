...
struct Apps {
  ...
  char cmd[300];
            ...
            {"Pax -w", "pax -w -f temp.pax big cpp src tst x86 etc; rm -f temp.pax"},
	    {"Grep", "grep '\\(.*a.*\\)\\1' lcc-3.5-freebsd.patch "
	     "lcc-3.5-freebsd.patch lcc-3.5-freebsd.patch > temp; rm -f temp"},
	    {"Cksum", "cksum * * * * * * * * * * * > temp; rm -f temp"},
	    {"Tsp", "tsp > temp; rm -f temp"},
	    {"Sor", "sor > temp; rm -f temp"},
	    {"Wc", "wc big big big big big big big > temp; rm -f temp"},
	    {"Compile", "gcc -pipe -c cpp/c.c; gcc -pipe -c cpp/cpp.c; gcc -pipe -c cpp/eval.c; gcc -pipe -c cpp/getopt.c; gcc -pipe -c cpp/hideset.c; gcc -pipe -c cpp/include.c; gcc -pipe -c cpp/lexer.c; gcc -pipe -c cpp/macro.c; gcc -pipe -c cpp/nlist.c; gcc -pipe -c cpp/tokens.c; gcc -pipe -c src/mips.c; rm -f *.o"}, 
            {"Uncompress", "cp big b.gz; gzip -d b.gz; rm -f b.gz b"},
	    {"Compress", "gzip -c big > big.gz; rm -f big.gz"},
};
