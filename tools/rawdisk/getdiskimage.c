...
/*
   ...
   if ((filefd = open (argv[1], (O_WRONLY | O_CREAT))) < 0) {
...
*/
   ...
   fprintf (stderr, "serving file from port %d\n", portno);
   ...
   while ((ret = read (newfd, &buf[0], 4096)) > 0) {
      filesize += ret;
      bytes += ret;
      sum = inet_sum ((unsigned short *) buf, ret, sum, 0);

      ret2 = write (1, buf, ret);
/*
      ret2 = write (filefd, buf, ret);
*/
      if (ret2 != ret) {
         fprintf (stderr, "Non-matched amount of data written: %d != %d\n", ret2, ret);
         exit (0);
      }

      if (bytes >= 4194304) {
         fprintf (stderr, "bytes: %7d,	(sum %x)\n", bytes, sum);
         bytes = 0;
      }
   }
