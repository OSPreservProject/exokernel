...
#define PCI_DEVICE_ID_NCR_53C810AP	0x0005
...
/* Size of memory pool for pci_malloc */
#define PCI_NUMDEVS 10
#define PCI_MEMSIZE PCI_NUMDEVS*sizeof(struct pci_dev)+NBPG
...
struct pci_dev {
	...
	/*
	 * For configured devices, this holds the device's data
	 */
	void		*devdata;
};
...
struct pci_dev_info {
	...
	unsigned char	bridge_type;	/* bridge type or 0xff */
					...
					/* attach function */
	void		(*attach)(struct pci_dev *, const char *);
};
...
extern struct pci_dev_info *pci_lookup_dev (unsigned int vendor,
					    unsigned int dev);
extern struct pci_dev *pci_find_dev (unsigned int vendor, unsigned int device);
