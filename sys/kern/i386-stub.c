...
void exceptionHandler(int exception_num, void *exception_addr)
{
  /* 
   * setgate(gate, istrap, ss, off, dpl) 
   */
  setgate (idt[exception_num], 0, GD_KT, ((void(*)(void))exception_addr), 3);
}


int kdebug_gdb_init(void)
{
  set_debug_traps();
  /*   putDebugChar ('+');  */
  return 1;
...
#define I386_STUB_STACKSIZE 10000
int remcomStack[I386_STUB_STACKSIZE/sizeof(int)];
static int* stackPtr = &remcomStack[I386_STUB_STACKSIZE/sizeof(int) - 1];
...
#define GDBBREAKPOINT() asm("   int $3");
...
/*     printf("<about to getDebugChar in putpacket>"); */
...
/* return a pointer to the last char put in buf (0) */
...
void handle_exception(int exceptionVector)
{
  ...
  char * ptr;
  int    newPC;
  extern void V0e (void);

#if STUB_DEBUG
  printf("entering handle_exception intr=%d vector=%d recursion level=%d...\n", 
	 intr_enabled(), exceptionVector, in_exception_handler);
#endif

  kdebug_flush_input_buffer();

  if (in_exception_handler)
    panic("Recursive debugger exception detected.");

  in_exception_handler++;


/*   exceptionHandler (14, _catchException14); */
  setgate (idt[14], 0, GD_KT, _catchException14, 0);
  ...
  while (1==1) {
    ...
    switch (remcomInBuffer[0]) {
     ...
     case 's' :
	  ...
	  setgate (idt[14], 0, GD_KT, V0e, 0);
/* 	  exceptionHandler(0xe, (void *)V0e); */
	  in_exception_handler--;
#if STUB_DEBUG
	  printf("returning in_ex_handler = %d\n", in_exception_handler);
#endif
  ...
  /* i dont think we ever reach here */
  assert(0); 
}
...
void set_debug_traps(void)
{
  ...
  stackPtr  = &remcomStack[I386_STUB_STACKSIZE/sizeof(int) - 1];

/*   exceptionHandler (0, _catchException0); */

  exceptionHandler (1, debug_trap);
  exceptionHandler (3, breakpoint_trap);
...
/*   exceptionHandler (16, _catchException16); */
  ...
  initialized = 1;
#if STUB_DEBUG
  printf("set_debug_traps exiting...\n");
#endif
}
