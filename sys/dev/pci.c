 ...
 */
/* April 1996 -- Ported to x86 exokernel by Elliot Waingold  */
...
void ncr_attach(struct pci_dev *, const char *);
void tulip_pci_attach(struct pci_dev *, const char *);
void ata_pciattach(struct pci_dev *, const char *);
...
#define DEVICE(vid,did,name) \
  {PCI_VENDOR_ID_##vid, PCI_DEVICE_ID_##did, (name), 0xff, 0}

#define CDEVICE(vid,did,name,func) \
  {PCI_VENDOR_ID_##vid, PCI_DEVICE_ID_##did, (name), 0xff, func}

#define BRIDGE(vid,did,name,bridge) \
  {PCI_VENDOR_ID_##vid, PCI_DEVICE_ID_##did, (name), (bridge), 0}
...
struct pci_dev_info dev_info[] = {
	...
	CDEVICE( NCR,		NCR_53C810,	"53c810",	ncr_attach),
	DEVICE(  NCR,		NCR_53C820,	"53c820"),
	CDEVICE(  NCR,		NCR_53C825,	"53c825",	ncr_attach),
	CDEVICE( NCR,		NCR_53C815,	"53c815",	ncr_attach),
	CDEVICE( NCR,		NCR_53C810AP,	"53c810AP",	ncr_attach),
	CDEVICE( NCR,		NCR_53C860,	"53c860",	ncr_attach),
	CDEVICE( NCR,		NCR_53C875,	"53c875",	ncr_attach),
	CDEVICE( NCR,           NCR_53C875J,    "53c875J",      ncr_attach),
	...
	CDEVICE( DEC,		DEC_TULIP_FAST,	"DC21140",	tulip_pci_attach),
	DEVICE( DEC,		DEC_FDDI,	"DEFPA"),
	CDEVICE( DEC,		DEC_TULIP_PLUS,	"DC21041",	tulip_pci_attach),
...
#ifdef ENABLE_IDE
	CDEVICE( INTEL,		INTEL_82371_1,	"82371 Triton PIIX IDE", ata_pciattach),
	CDEVICE( INTEL,		INTEL_82371SB_IDE, "82371SB IDE (PIIX3)", ata_pciattach),
	CDEVICE( INTEL,          INTEL_82371AB_IDE, "82371AB IDE (PIIX4)", ata_pciattach),
#else
	DEVICE( INTEL,          INTEL_82371_1,  "82371 Triton PIIX IDE"),
        DEVICE( INTEL,          INTEL_82371SB_IDE, "82371SB IDE (PIIX3)"),
        DEVICE( INTEL,          INTEL_82371AB_IDE, "82371AB IDE (PIIX4)"),
#endif
...
/* 
 * Search for device (vendor, device) on the list of found PCI devices.
 * Assumes that pci_devices has already been filled in.
 */

struct pci_dev *
pci_find_dev (unsigned int vendor, unsigned int device)
{
  struct pci_dev *trav;
  ...
  while (trav)
    {
      if ((trav->vendor == vendor) && (trav->device == device))
	return trav;
      else
	trav = trav->next;
    }
 ...
 */
unsigned char pci_mempool[PCI_MEMSIZE];  /* ELW added this */
static void *pci_malloc(long size, unsigned long *mem_startp)
{
	void *mem;

#ifdef DEBUG
	printk("...pci_malloc(size=%ld,mem=%p)\n", size, (void *) *mem_startp);
#endif  
	if ((unsigned long)&mem_startp+size > (unsigned long)pci_mempool+PCI_MEMSIZE)
	  {
	    panic("pci_malloc: out of memory in PCI pool");
	  }
	else
	  {
	    mem = (void*) *mem_startp;
...
static unsigned int scan_bus(struct pci_bus *bus, unsigned long *mem_startp)
{
	...
	for (devfn = 0; devfn < 0xff; ++devfn) {
		...
		if (!info) {
			printk("Warning : Unknown PCI device (%x:%x).  "
			       "Please read sys/xok/pci.h \n",
		...
		if (info) {
			if (info->attach) {
				info->attach(dev, info->name);
			}
#if 0
#endif
			else {
				printk("%s device found, not configured\n",
					info->name);
			}
		}
	}
...
void pci_init ()
{
	unsigned long mem_start = (unsigned long) pci_mempool;

#ifdef PCI_DEBUG
	printk("   initializing PCI BIOS\n");
#endif
	pcibios_init();
