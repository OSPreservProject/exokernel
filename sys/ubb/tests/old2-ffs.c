...
void ffs_write_db(struct xn_op *op, int i, db_t old_db, cap_t cap, db_t new_db) {
...
void ffs_db_alloc(struct xn_op *op, int index, db_t db, cap_t cap) {
	ffs_write_db(op, index, db, cap, db);
}
/* Free a direct block. */
...
int xn_import(struct udf_type *t) {
	...
	nblocks = roundup(sizeof *t, XN_SECTOR_SIZE) / XN_SECTOR_SIZE;
	db = 0;
	...
	for(i = 0; i < nblocks; i++) {
		...
		p = (char *)t + i * PAGESIZ;
		nbytes = (i == (nblocks - 1)) ? 
				(sizeof *t % XN_SECTOR_SIZE) : PAGESIZ;
...
#       define size(field) 	(sizeof ((struct inode *)0)->field)
	...
	xn_begin(&t, "FFS Inode Meta", XN_NOT_STICKY);
	   ...
	   /* type field that we discriminate on. */
           xn_fixed_seq(&t, offset(type), sizeof(type), 1, XN_BYTES);
	...
	xn_begin(&t, "FFS Inode", XN_NOT_STICKY);
	   ...
	   /* XXX: UID/GID control the entire inode.  Need to automate. */
           xn_fixed_seq(&t, offset(uid), size(uid), 1, XN_BYTES);
           xn_fixed_seq(&t, offset(gid), size(gid), 1, XN_BYTES);
           ...
           xn_fixed_seq(&t, offset(db[0]), sizeof(db_t), NDIRECT, XN_DB);
           xn_fixed_seq(&t, offset(s_indir), sizeof(db_t), 1, s_indir_t);
           xn_fixed_seq(&t, offset(d_indir), sizeof(db_t), 1, s_indir_t);
#if 0
           xn_fixed_seq(&t, offset(d_indir), sizeof(db_t), 1, d_indir_t);
#endif
           xn_fixed_seq(&t, offset(t_indir), sizeof(db_t), 1, t_indir_t);


	xn_end(&t, sizeof(struct inode));
